...
Original file is located at
    https://colab.research.google.com/drive/1yxtMYdE6h-i_sY2nQN3VeQHQeZszuBgI
...
The **update step** in normal stochastic gradient descent can be represented as
...
The formula of updates is further expanded upon with different optimization models, but the base is as listed above.The end goal of repeated updates is to minimize loss, measured by formulae like MSE, and to minimize variance ($\sigma^2$) which leads to a better fit to the training data after the split.
...
>$ m+1$ is the biased corrected first moment estimate at time $t+1$

>$v_{t+1} $ is the biased corrected second moment estimate at time $t + 1$

>$\eta$ is the learning rate

>$\epsilon$  is a small constant added for numerical stability. Consider it similar to noise, which is just here for stabilization. Usually, and in this case, it is $10^{-8}$

# Loss Optimization Function 3: RMSProp (Root Mean Square Propogation)

RMSProp is very similar to ADAM, with the largest difference being the exclusion of the first moment estimate in RMSProp, which is represented in the update step.
The formulae are relatively the same which are as follow:

## Second Moment Estimate
As with ADAM, the second moment estimate can be represented with:

> $v_t+1 = \beta v_{t} + (1-\beta) \cdot (\nabla J (\theta_t))^2$

Where:

> $v_{t}$ represents the biased corrected second moment estimate at time ${t}$


> $\beta$ is the exponential decay factor for the moving average's squared gradients

> $1-\beta$ is the factor by which the square of the current gradient is weighted
...
The update function differs very slightly from the ADAM one, due to the difference of moment estimates considered.

Here, the update function is represented as:
...
The major difference from ADAM is the lack of an $m$ variable, since we don't have a first moment estimation.

The overall purpose of the part of the RMSprop and ADAM formulae is to update the parameters in a way that adapts the learning rates for each parameter based on the history of squared gradients, helping to achievemore stable and eﬀicient convergence during the optimization process. This serves to replace the SGD formula, which while effective, is ineffcient and can lead to vanishing gradient problems.
"""